"""Demo version of IT Query

Uses sample data from data.json

Bionumerics specific functions removed
...
#sample data file
CACHE_FILE = os.path.join(SCRIPT_PATH, 'data.json')
...
#Jana's module to update ItemTracker
#import New_GetTubesForStrainsInBNAndUpdate as lims

#don't use bn functions in demo
#import bn
#import pgdb
...
def from_json(json_object):
	"""Format datetime object from cache file."""
	if '__class__' in json_object:
		if json_object['__class__'] == 'datetime.datetime.isoformat':
			return datetime.datetime.strptime(json_object['__value__'],
			'%Y-%m-%dT%H:%M:%S')
	return json_object
...
class Form(QMainWindow, Ui_MainWindow):
	...
	def __init__(self, initIds, initErrIds, parent=None):
		...
		with open(CACHE_FILE) as f:
			self.cache = json.load(f, object_hook=from_json)

		self.selectedIds = self.cache["selectedIds"]
		...
		LOG_FNAME = "{0}\{1}-{2}.{3}".format(SCRIPT_PATH, "itquery",
											 runDate, "txt")
...
#		self.itemIds = {}
		self.itemIds = self.cache["itemIds"]
		self.itemNames = {}
		# bnFields - bionumerics information in the info panel
#		self.bnFields = {}
		self.bnFields = self.cache["bnFields"]
#		self.strainInfo = {}
		self.strainInfo = self.cache["strainInfo"]
		# maintains list of keys that are updated
		self.updatedStrains = []
		self.selectRadioButton.setChecked(True)
#		self.itemProperties = {}
		self.itemProperties = self.cache["itemProperties"]

#		self.tableResults = []
		self.tableResults = self.cache["tableResults"]
#		self.getBnFields()
...
#				self.mkError("Key: {0}, {1}".format(key, message))
		self.itemIdsAll = self.cache["itemIdsAll"]
		self.database = None
#		self.updateUi()
...
#			fieldNames = bn.getFieldNames()
			fieldNames = self.cache["fieldNames"]
...
#		(dnaData, noData, errors) = lims.GetNewestDNAForCherryPicking(itemIds)
		dnaData, noData, errors = self.cache["dnaData"], [], []
		if len(noData):
...
#		cherryData = pgdb.get_data(db)
		cherryData = self.cache["cherryData"]
	...
	def getSaveFile(self, caption=""):
		...
		fn = QFileDialog.getSaveFileName(self, caption, SCRIPT_PATH,
...
#			try:
#				self.tableResults = \
#				lims.SelectAllselectedEntries(self.itemIdsAll.values())
#			except:
#				pass
		else:
...
#		message) = lims.GetDictionary((self.selectedIds.keys()))
		...
		(treeData, self.itemProperties,
		 message) = self.cache["treeData"], self.cache["itemProperties"], []
...
#			bn.selectEntries(self.selectedIds.values() + self.errorIds.keys())
	...
	def sorry(self):
		"""For functions that will not work in demo."""
		self.mkInfo("Sorry. This feature is not implemented in the demo :(")
		...
		if parentName:
			...
			if itemId:
				...
				for field, value in (("Strain", parentName),
					...
					count += 1
			try:
				properties = self.itemProperties[unicode(int(itemId))]
			except KeyError:
